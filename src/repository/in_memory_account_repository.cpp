...
bool InMemoryAccountRepository::verifyCode(const std::string& email, const std::string& code, VerificationCodeType type) {
    ...
    const auto& verificationCode = it->second;
    ...
    auto duration = std::chrono::duration_cast<std::chrono::minutes>(now - verificationCode.createdAt);
    if (duration.count() > 5) {
...
std::string InMemoryAccountRepository::generateRandomCode(int length) {
    ...
    for (int i = 0; i < length; ++i) {
        code[i] = charset[dis(gen)];
    }
    
    return code;
}

std::string InMemoryAccountRepository::createAuthToken(const std::string& accountId) {
    std::lock_guard<std::mutex> lock(mutex);
